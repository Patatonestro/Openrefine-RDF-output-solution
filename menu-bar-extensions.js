...
RdfExporterMenuBar.exportRDF = function(format, ext) {
    if (!theProject.overlayModels.rdfSchema) {
        ...
        Refine.wrapCSRF(function(csrfToken) { 
            let form = RdfExporterMenuBar.rdfExportRows(format, true, ext, csrfToken);
...
RdfExporterMenuBar.rdfExportRows = function(format, includeEngine, ext, csrfToken) {
        ...
        $('<input />')
            ...
            .val(format)
            .appendTo(form);
        $('<input />').attr("name", "quoteAll").appendTo(form); 
        if (includeEngine) {
        $('<input />').attr("name", "engine").val(JSON.stringify(ui.browsingEngine.getJSON())).appendTo(form);
        }
        return form;
